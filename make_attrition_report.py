#!/usr/bin/env python3
"""
hb_loss_report_min.py
Create a minimal Markdown report for handbook processing data loss.

Sections:
  - Drop between stages table
  - Missing PDFs
  - No EB pages (given PDF)
  - No CSV (given EB pages)
  - No reliable EB rows (given CSV)

Input  (from --series & --in_dir):
...
def main():
    ...
    in_path  = (Path(args.in_dir).expanduser().resolve()  /
                f"{series}_handbook_processing_loss.dta")
    out_path = (Path(args.out_dir).expanduser().resolve() /
                f"{series}_hb_processing_report.md")
    ...
    s_id  = f"{series}_state_id"
    s_nm  = f"{series}_state_name"
    d_id  = f"{series}_district_id"
    d_nm  = f"{series}_district_name"
    ...
    drops = [
        total - keeps[0],             # total → pdf
        keeps[0] - keeps[1],          # pdf → eb pages
        keeps[1] - keeps[2],          # eb pages → csv
        keeps[2] - keeps[3],          # csv → reliable rows
    ]

    lines = []
    lines.append(f"# {series.upper()} Handbook Processing — Minimal Data Loss Report\n")
    lines.append(f"_Input_: `{in_path}`  \n_Output_: `{out_path}`\n")

    lines.append("## Drop Between Stages\n")
    lines.append("| Between Stages | Dropped |")
    lines.append("|---|---:|")
    lines.append(f"| total → PDF present | {drops[0]} |")
    lines.append(f"| PDF present → EB pages found | {drops[1]} |")
    lines.append(f"| EB pages found → CSV extracted | {drops[2]} |")
    lines.append(f"| CSV extracted → Reliable EB rows | {drops[3]} |\n")

    # ---------- Drill-downs ----------
    def table_block(title, frame):
    ...
    table_block("Missing PDFs", missing_pdf)
    table_block("No EB pages (given PDFs)", no_eb_pages)
    table_block("No CSV (given EB pages)", no_csv)
    table_block("No reliable EB rows (given CSV)", no_rows)
