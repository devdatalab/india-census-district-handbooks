...
| `llm_csv_hb_extractor.py` | Uses Gemini 2.5 Flash to extract clean, concatenated CSVs from EB-page PDFs. Requires `prompt_template.txt` for prompt formatting and `extract_log.csv` for progress tracking and checkpointing: manually flip `recreate_flag` to 1 in `extract_log.csv` to force the LLM to re-run for a file and set `recreate_flag` back to 0 on success; on exception it set `error_flag` and `recreate_flag` to 1 so next run tries again. |
